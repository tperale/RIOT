...
void sx128x_cmd_set_tx(const sx128x_t *dev, uint8_t period_base, uint16_t period_base_count) {
    DEBUG("[sx128x] Cmd set tx with period base %#02x and count %#02x\n", period_base, period_base_count);
...
void sx128x_cmd_set_packet_type(sx128x_t *dev, uint8_t packet_type) {
    DEBUG("[sx128x] Set packet type: %d\n", packet_type);
    if (packet_type != SX128X_PACKET_TYPE_LORA) {
        DEBUG("[sx128x] LoRa packet type supported only");
    }
...
void sx128x_cmd_set_tx_params(const sx128x_t *dev, int8_t power, uint8_t ramp_time) {
    if (power < -18 || power > 13) {
        DEBUG("[sx128x] Out of range power range")
        return;
    }
...
void sx128x_cmd_set_packet_params(sx128x_t *dev, uint8_t param1, uint8_t param2, uint8_t param3, uint8_t param4, uint8_t param5, uint8_t param6, uint8_t param7) {
    ...
    switch (dev->settings.modem) {
        ...
        default:
            DEBUG("[sx128x] Not supported packet type\n");
            return;
    }
}
...
void sx128x_set_state(sx128x_t *dev, uint8_t state)
{
    ...
    if (IS_ACTIVE(ENABLE_DEBUG)) {
        switch (state) {
        case SX128X_RF_IDLE:
            DEBUG("[sx128x] Change state: IDLE\n");
            break;
        case SX128X_RF_RX_RUNNING:
            DEBUG("[sx128x] Change state: RX\n");
            break;
        case SX128X_RF_TX_RUNNING:
            DEBUG("[sx128x] Change state: TX\n");
            break;
        default:
            DEBUG("[sx128x] Change state: UNKNOWN\n");
            break;
        }
    }
...
{
    (void)(dev);
    DEBUG("[sx128x] Set channel: %" PRIu32 "\n", channel);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set max payload len: %d\n", maxlen);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set spreading factor: %d\n", datarate);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set coding rate: %d\n", coderate);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set fixed header length: %d\n", fixed_len);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set payload len: %d\n", len);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set preamble length: %d\n", preamble);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set RX timeout: %" PRIu32 "\n", timeout);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set TX timeout: %" PRIu32 "\n", timeout);
...
{
    (void)(dev);
    DEBUG("[sx128x] Set symbol timeout: %d\n", timeout);
}
